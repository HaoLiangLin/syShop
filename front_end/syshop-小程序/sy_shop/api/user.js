...
export function uploadIcon(data) {
  return request.post('/users/uploadIcon', data)
}
...
export function checkIsNotUpdateUsername() {
  return request.get('/uses/isNotUpdateUsername')
}
...
// 修改个人信息
export function updateUserInfo(id, fullName, gender, age, email, birthday, qq) {
  return request.put('/userinfo/update', { id, fullName, gender, age, email, birthday, qq })
}
...
export function queryBill(year, month, day) {
  return request.get('/account/bill', {
    params: {
      year,
      month,
      day
...
// 修改购物车
export function updateShoppingCart(id, uid, gid, quantity) {
  return request.put('/shoppingCart/update', { id, uid, gid, quantity })
}
...
// 取消收藏
export function deleteCollection(uid, gid) {
  return request.delete('/collection/delete', { uid, gid })
}
